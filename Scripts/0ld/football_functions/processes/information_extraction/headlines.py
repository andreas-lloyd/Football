...
'''
import pickle, os, re
import football_functions.source_specific.bbc as bbc # then bbc.process_html.XXX
import football_functions.source_specific.dailymail as dailymail
import football_functions.source_specific.mirror as mirror
import football_functions.source_specific.guardian as guardian
import football_functions.source_specific.skysports as skysports
import football_functions.source_specific.telegraph as telegraph

def extract_headlines(html_loc, story_loc, date_today, domain):
    '''
    ...
    if not os.path.exists(pickle_path):
        print('Making directory {}'.format(pickle_path))
        os.makedirs(pickle_path)

    # Then load in the HTML content
    print('Getting content from {}'.format(html_loc))
    ...
    if domain == 'bbc':
        ...
        articles_info = dailymail.extract_dailymail(html_content, modifier)
        
    elif domain == 'theguardian':
        articles_info = guardian.extract_guardian(html_content)
        
    elif domain == 'mirror':
        articles_info = mirror.extract_mirror(html_content)
    
    elif domain == 'skysports':
        modifier = 'regional' in html_loc
        articles_info = skysports.extract_skysports(html_content, modifier)
        
    elif domain == 'telegraph':
        articles_info = telegraph.extract_telegraph(html_content)

    print('Finished pulling articles - now saving article content')
...
def process_html(html_loc, story_loc, date_today):
    ...
    for domain in os.listdir(html_loc):
        ...
        for link_type in link_types:
            ...
            for html_page in os.listdir(domain_loc + link_type):
                html_loc = domain_loc + link_type + '/' + html_page
                articles_info = extract_headlines(html_loc, story_loc, date_today, domain)
